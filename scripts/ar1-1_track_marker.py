            ...
            for msg in msg.markers:
                ...
                if msg.id == TARGET_ID:
                    ...
                    if self.sq4_turn_to_marker_end == True and self.sq5_approach_2_marker_end == False:
                        self.stop_move()
                        #print "ar_pose.theta = %5s, ar_pose.y = %5s, ar_pose.x = %5s" %(self.ar_pose.theta, self.ar_pose.y, self.ar_pose.x)
                        '''
                        pose.x: 0.09~0.14 ---> stop
                        pose.theata: > abs(0.15) ----> ctrl tw.angular.z
                        '''
                        #'''
                        if   self.ar_pose.y >=  0.025:
                            self.tw.angular.z = -ANG_SPD * 0.25
                        elif self.ar_pose.y <= -0.025:
                            self.tw.angular.z =  ANG_SPD * 0.25
                        else:
                        ...
                        if   self.ar_pose.x > 0.15:
                            self.tw.linear.x =  LIN_SPD * 0.5
                        elif self.ar_pose.x > 0.12:
                            self.tw.linear.x =  LIN_SPD * 0.25
                        elif self.ar_pose.x < 0.09:
                            self.tw.linear.x = -LIN_SPD * 0.25
                        else:
                        ...
                        if self.ar_pose.x > 0.09 and self.ar_pose.x < 0.12:
                            if self.ar_pose.y > -0.025 and self.ar_pose.y < 0.025:
                                self.sq5_approach_2_marker_end = True
                                print "sq5_approach_2_marker_end"
                                print "================="
