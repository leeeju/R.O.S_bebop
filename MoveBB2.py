...
from bb2_pkg.msg import Pos_XYZ_th              #x,y,z 의 값을 말해줌

LIN_SPD = 0.125     # 이동 속도값
ANG_SPD = 0.50      # 이동 속도값
'''
해당 클라스에는 이륙 착륙, 좌측, 우측, 직진, 후진의 대한 코드가 있으며 해당 자료를 섭스크라이브 해서 가져가면 된다.
'''
class MoveBB2:

    def __init__(self):
        rospy.Subscriber('/bb2_pose_odom', Pos_XYZ_th, self.get_pos_xyzth_cb)
        self.pub0 = rospy.Publisher('/bebop/cmd_vel', Twist, queue_size = 1)     # 방향
    ...
    def move_y(self, distance, tolerance):                                    # y 축 이동
        tw = Twist()
    ...
    def move_z(self, height, tolerance):                                        # z 축 이동
        tw = Twist()
